...
const DateValidation = ({ reviewDate, expiredDate }) => {
  ...
  const isReviewDateBeforeExpiredDate =
    moment(reviewDate).isBefore(expiredDate);
  const isSameDay = moment(reviewDate).isSame(expiredDate, "day");

  if (
    !isReviewDateBeforeExpiredDate ||
    (isSameDay && moment(reviewDate).isSameOrAfter(expiredDate, "hour"))
  ) {
    return (
      <div
        style={{
